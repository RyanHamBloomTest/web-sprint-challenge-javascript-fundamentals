...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
/* The external variable is declared in the scope outside myFunction, so myFunction is able to access it as well as anything within myFunction. Similarly, internal is defined within the scope of myFunction, so anything inside that function, including nested functions like nestedFunction, will be able to access it.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(animals){
    const namesArray = []
    animals.forEach((animal) => namesArray.push(`name: ${animal.animal_name}, scientific: ${animal.scientific_name}`));
    return namesArray;
  }
  ...
  */
  ...
  function USApop(animals){
    return animals.reduce((previous, current) => previous + current.population, 0);
  }
